...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--r2g', action='store_true',
        help='Enable the red-to-green book (research/fuckup_audit/H/F6, 2026-09-17), the'
             ' same engine as --hod with book=red_to_green. OFF by default — config.yaml'
             ' red_to_green.enabled must be true (and dry_run false) to actually trade.'
    )
...
def run_scan(config, verbose: bool = False, trade: bool = False,
             ...
             enable_orb: bool = False, enable_hod: bool = False, enable_r2g: bool = False) -> None:
    ...
    # Red-to-green engine (2026-09-17, research/fuckup_audit/H/F6): the SAME engine class with book=red_to_green —
    # its own DB strategy tag, bar-handler id, [R2G] log tag and config block; gates internal (enabled/dry_run).
    r2g_engine = None
    if trade and enable_r2g:
        try:
            from trading.hod_break_engine import HodBreakEngine
            r2g_engine = HodBreakEngine(
                alpaca_client=alpaca, db=db, stop_monitor=stop_monitor,
                notifier=notifier, cfg=config.red_to_green_cfg,
                order_stream=order_stream)
            if stop_monitor is not None and not stop_monitor.polling_mode:
                r2g_engine.register_on_stop_monitor()
            r2g_engine.sync_positions()
            if r2g_engine.enabled:
                r2g_engine.start_drain_thread()
            logger.info(f"Red-to-green strategy loaded — master_flag={r2g_engine.enabled}, "
                        f"dry_run={r2g_engine.dry_run}")
        except Exception as e:
            logger.error(f"Red-to-green engine init failed ({e}) — red-to-green off")
            r2g_engine = None
...
def main() -> None:
    ...
    if args.scan:
        run_scan(config, verbose=args.verbose, trade=args.trade,
                 ...
                 enable_orb=args.orb, enable_hod=args.hod, enable_r2g=args.r2g)
