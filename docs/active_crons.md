...
> - `1e0fd258` recurring weekdays 21:52 UTC (7-day expiry, re-arm Saturday) — HOD-BREAK EOD CHECK: runs `scripts/hod_break_eod_check.py` (journal signals → exact-spec re-run on today's bars → level/stop/minute parity, spec exits + R, would-be/realized book P&L, "spec has no trade" defects), rolling tally since 9/14 vs the study's ~35/wk +0.16R, 15:55 flat verification, `HOD-break tick raised` must be 0; one `[HOD EOD]` telegram with ESCALATIONS; read-only.
> - The DAILY EOD DEEP DIVE (1bdf606c) and OWNER BRIEF (28b924e7) still cover BF/ORB; HOD-break has its own two crons above.
> - `2b442753` recurring weekdays 12:29 UTC once per session (re-created 9/14 with a tighter kill-rail pattern; was 1688079e) (replaced hourly e3f086dc on 9/14 — a persistent Monitor lives all session, hourly re-arms duplicated it; 7-day expiry) — ARM THE HOD-BREAK LIVE WATCH: starts ONE persistent Monitor if none is running on `journalctl -u onemil-trader -f` filtered to `[HOD`/tick errors/Tracebacks/force close/BF+ORB fills; a defect telegrams `[HOD WATCH]` immediately; first dry signal of the day telegrams once; read-only, never orders.
> - System crontab `57 19 * * 1-5 scripts/hod_break_deadman_flat.py` (9/14): when the trader is NOT active at the close, sells OUR shares of every DB-open hod_break row (marketable limit off the bid), marks exit_pending_verification, telegrams `[HOD DEAD-MAN]`. Never touches other positions.
