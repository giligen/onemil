...
> - `e3f086dc` recurring weekdays hourly 12:29–20:29 UTC (7-day expiry) — ARM THE HOD-BREAK LIVE WATCH: each fire starts a 58-min persistent Monitor on `journalctl -u onemil-trader -f` filtered to `[HOD`/tick errors/Tracebacks/force close/BF+ORB fills; a defect telegrams `[HOD WATCH]` immediately; first dry signal of the day telegrams once; read-only, never orders.
