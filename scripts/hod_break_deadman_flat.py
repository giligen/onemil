...
strategy's or the owner's shares.

The rows, the positions and the client-order id are STRATEGY-SPECIFIC, so the script takes the same `--book` option
as the EOD check: `--book red_to_green` flattens the red-to-green book's rows (DB strategy `red_to_green`, `r2g-dm-`
client ids, `[R2G DEAD-MAN]` telegram) and touches nothing of the HOD-break's. Default `hod_break` — byte-identical
to the pre-option script. Both books need their own run.

Usage: python3 scripts/hod_break_deadman_flat.py [--force] [--now HH:MM-ET] [--book hod_break|red_to_green]
"""
import json
import os
import subprocess
...
def main() -> int:
    argv = list(sys.argv[1:]); book = load_book(book_from_argv(argv))
    ...
    if '--now' in argv:
        hh, mm = argv[argv.index('--now') + 1].split(':'); now_et = now_et.replace(hour=int(hh), minute=int(mm))
    ...
    for r in rows:
        ...
        try:
            ...
            coid = f"{book.coid_prefix}-dm-{sym}-{today[5:]}"[:48]
    ...
    try:
        subprocess.run([sys.executable, 'scripts/send_telegram_alert.py', f'[{book.tag} DEAD-MAN] trader inactive at the close: {n_ok} of {len(rows)} open {strategy} position(s) handled, see logs/hod_deadman.log'], check=False)
    except Exception:
        pass
