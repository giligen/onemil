...
rem Upgrade PROD (live trading) onemil node to c6i.8xlarge for heavy BT.
rem Pass /y to skip the confirmation prompt.
rem WARNING: prod runs the live trader. Do NOT resize during US market hours
rem          (09:30-16:00 ET) without an explicit reason -- you will kill any
rem          open positions' monitoring during the 3-5 min downtime.
rem Cost: ~$1.36/hr -- remember to downgrade when done.

set REGION=eu-north-1
set TARGET_TYPE=c6i.8xlarge
...
set /p CONFIRM=Proceed with PROD upgrade? (y/N):
if /i not "%CONFIRM%"=="y" (
    echo Aborted.
    exit /b 1
)
...
echo.
echo Upgrade complete. SSM tunneling typically takes 60-90s to come online.
