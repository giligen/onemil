...
def generate_signals(
    ...
    # Clip to regular session (09:30 ET to 16:00 ET). The intraday bar cache
    # sometimes includes pre-market bars (08:30 ET+) and could include extended
    # hours (16:00 ET+) for certain symbols. Without this clip:
    #   - pre-market: `op = bars.iloc[0]['open']` would use the pre-market
    #     open as the day anchor, allowing pre-market crosses + pre-market
    #     entries that PROD would never take.
    #   - after-hours: a stock that didn't cross during regular session could
    #     "cross" in after-hours, creating fake trades.
    # PROD scanner only runs 09:30-16:00 ET. Fix introduced 2026-04-15.
    if 'ts' in bars.columns and len(bars) > 0:
        bars_ts = pd.to_datetime(bars['ts'], utc=True)
        et_dt = bars_ts.dt.tz_convert(ET)
        # Regular session: 09:30 ET (inclusive) → 16:00 ET (exclusive).
        regular = (
