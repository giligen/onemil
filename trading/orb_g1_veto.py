...
def g1_reject(return_volatility_20d,
              ...
              pdr_min: float = DEFAULT_PDR_MIN,
              short_history_veto: bool = False) -> Optional[str]:
    ...
    KEEP (return None) when:
      * return_volatility_20d is None / NaN               (fail-open; missing)
      * return_volatility_20d == 0.0 and not short_history_veto (8/15 fail-open)
    ...
    if _is_missing(rv):
        return None
    if rv == 0.0:
        # The "history too short to certify" marker (< 5 prior daily bars).
        # 2026-09-06 V1 veto study (research/orb_veto_study/REPORT.md): on the
        # honest B+ book every short-history pick lost (4 fills, 0 winners,
        # -$340 at $10K; 3 more burned a slot unfilled) — a listing days old
        # has no day-2 continuation to monetize. `short_history_veto=True`
        # (orb.yaml g1_veto.short_history_veto) vetoes the marker; False keeps
        # the 8/15 fail-open behaviour.
        if short_history_veto:
            return "short history (rv20==0.0 marker: < 5 prior daily bars)"
        return None
    if _is_missing(pdr):
        return None
