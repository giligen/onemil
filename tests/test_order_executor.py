...
class TestSubmitMarketBracketOrder:
    """Tests for market-like bracket order submission (thin-liquidity flow)."""

    def test_submit_market_bracket_order(self, executor, mock_alpaca, db):
        """Verify limit price = current_price * 1.005, SL/TP correct, trade saved."""
        mock_alpaca.submit_bracket_order.return_value = {
            'id': 'mkt-order-1',
            'status': 'accepted',
            'symbol': 'TEST',
        ...
        plan = _make_plan()
        current_price = 4.42
        result = executor.submit_market_bracket_order(plan, current_price)

        assert result is not None
        assert result['order_id'] == 'mkt-order-1'
        assert result['status'] == 'accepted'
        assert result['order_type'] == 'market_bracket'
        assert result['current_price'] == 4.42
        assert result['limit_price'] == round(4.42 * 1.005, 2)

        # Verify Alpaca called with correct params
        mock_alpaca.submit_bracket_order.assert_called_once_with(
            symbol='TEST',
            qty=113,
            side='buy',
            limit_price=round(4.42 * 1.005, 2),
            tp_price=4.90,
            sl_price=4.29,
        )
        ...
        assert len(trades) == 1
        assert trades[0]['symbol'] == 'TEST'
        assert trades[0]['order_id'] == 'mkt-order-1'
        assert trades[0]['entry_price'] == 4.42  # current_price, not breakout
        assert trades[0]['shares'] == 113
    ...
    def test_handles_api_failure(self, executor, mock_alpaca):
        ...
        plan = _make_plan()
        result = executor.submit_market_bracket_order(plan, 4.42)
        assert result is None
