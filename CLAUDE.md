...
|---|---|---|---|
| Bull flag (P1 profile) | `config.yaml trading.enabled` | **PAUSED 9/25 (owner GO, `enabled: false` at the 12:30 boot): no out-of-sample edge (2024H2 +0.10 R n 26 one-trade-carried; Jan–May 26 n 15 t 0.6, sign depends on the cost assumption — `research/exec_cost/REPORT_RECAL.md`), ~0.8 trades/wk cannot resolve it; wrapper guard 950dc4d stays** — history: PAUSED 9/14, LIVE 9/21–9/24, **risk cap 1.0× since 9/24 (owner GO): every trade risks ≤ $150 after all multipliers** (was $270–900 via conviction + MACD tiers); **2024H2 out-of-regime: +0.10 R on 26 trades, one trade carries it (`research/bf_2024/REPORT.md`)**, ramp L0→L3 on positive realized P&L (`docs/bf_p1_ramp.md`, `scripts/bf_ramp_check.py`) | honest book $107K/79 tr under the unified trail; **halves under measured NBBO cost** ($139K→$69K, VAL −$8K) — the ramp band must be rebuilt on the measured-cost book |
| ORB B+ | `orb.yaml strategy.enabled`, `--orb` | **CLOSED as a money book 9/25 (cell 1,426, `research/orb_latency_bt/REPORT.md`): every BT fill replayed on XNAS ticks with the engine's chase rule gives +0.028 R/fill out of sample at ZERO latency (+0.03 at 3–20 s, ≈ 0 at ≥ 30 s; only 21 % of fills match the BT's 30 bps entry model) — latency explained part of the live loss but the tape edge is ~$11/trade; stays PAUSED, no ramp, no rehearsal for it**; PAUSED 9/25 12:21 UTC (act-as-owner): live book 5/19–9/23 = −$5,281 on 123 closed fills (4 of 5 months red) vs a positive backtest for the same months; cause under repair = first order 26–49 s late (universe seed re-fetched in full every tick, `docs/orb_latency_fix_REPORT.md`, flag `execution.prewarm_seed`) + ~72 bps adverse ask-to-fill drift; resume only after the prewarm fix passes a weekend boot rehearsal and a dry run tracks the BT** — was LIVE 9/21–9/24 (catalyst-off, 8 slots, entry-drain thread, 50 bps); **add-on pools** `universe.addon_pools` (gap 4–5 % $3–30, gap 3–5 % $30–50; selection chain per pool, production first) **DRY DAY 9/22**, enable only on the owner's word (`research/orb_seed_wide/PREREG_LIVE_UNION.md`, kill rules there) | honest $6,085 / 21 mo at $10K stage; **out-of-regime tests (point-in-time universes, live config; RE-RUN 9/25 under the LIVE EXIT RULE after a silent bars-source fallback was found — `research/orb_verify/REPORT.md`, `research/orb_2023/REPORT_liveexit.md`): 2023-01..2024-06 FLAT +0.015 R/fill (n 106, t 0.30), 2024H2 +0.031 (n 59, t 0.38; ATR14 coverage 92 %), pooled +0.020 ± 0.043 R (t 0.48) — the untouched out-of-sample edge is ≈ 0; the earlier +0.089 / −0.007 were legacy-exit numbers vs **+0.105 R/fill at the SAME live config 2025-01..2026-09 (n 473, t 3.31, $18.7K at $375, ex-top-5 % ≈ 0)** — the old "+0.27 in 2025" was the catalyst-veto-ON book (`research/thermo/REPORT.md`); per fill ORB is ~+0.1 R in and out of regime, the regime changes FREQUENCY (1.4–2.3 vs 4.3–7.3 fills/wk); breakout-thermometer sizing FAILED (cells 1,420–1,421) (`research/orb_2023/REPORT.md`) — ramp ADVANCE needs 40 live fills (`orb_ramp_check.py min_fills_advance`); both add-on pools dry-only (p30 NEGATIVE out of regime)**; union rung BT +40 % / +60 % $ at a third of the per-trade edge (cell 1,328, independently rebuilt); weekly selection refit `scripts/orb_weekly_refit.py` (Sun 20:00 UTC); NEVER refit `adaptive_mults`, never drop the Q5 1.5 cap; corpse gate = bar older than 4 days (9/21 fix) |
| MACD wave | `onemil-macd-wave` service, `macd_wave.yaml` | **NOT INSTALLED on this node** (no unit file; not running since ≥ 8/16 — found 9/24; the EOD hygiene line flags it daily) | outlier-dependent P&L; filters tuned in-sample; reinstall only on the owner's word |
| HOD-break | `hod_break.enabled: true`, **`dry_run: true` again from Monday 9/28 (real orders ran 9/25 13:38–15:00 ET at $50: 2 fills, −$48) — cell 1,438, the live rule with CORRECT levels (bars_sip.db), is −0.21 R net (t −8, 82 % fill rate, `research/hod_entry/REPORT_1438.md`); the 1,427 +0.33 R 'fill filter' was a sparse-cache.db level ARTIFACT; the both-fill/'tracked' +0.25 R cohort is a FULL-DAY-RANGE LOOK-AHEAD from the backtest cache build (cell 1,444 not runnable, `research/hod_entry/RESULT_1444.md`; 1,438 corrected net ≈ −0.11 R, raw ≈ 0) — CLOSED at every executable entry on this population; forward instrument = scanner_qualified_at_arm in the dry ledger; weekend 9/26: 10 more cells (short mirror, gapper universe, prior-day high, exits, stop distance, sizing, override) ALL FAIL, measured stop slip 35 bps mean / 82 p90 (cell 1,443) → live expectation −0.22 R net; owner report `research/hod_entry/OWNER_REPORT_20260928.md`; **revival 9/26 (owner's ask): 23 cells 1,445–1,467, each independently rebuilt — CLOSED as a money book on this population (perfect-foresight ceiling +0.17 R; every causal predictor negative; execution fixes leave −0.05/−0.08 R; best joint +0.05 R t 1.1); surviving mechanism = stop-limit exit 20 bps below the stop (tape-verified slip 35 → ≈ 13 bps) → dry-run instrument first (`PREREG_1466.md`, `RESULT_1466.md`)** — `entry_mode: resting_stop_limit` stays for dry parity data — cells 1,423/1,427: the resting stop-limit's FILLS earn +0.29/+0.24/+0.33 R net (2025H2 / VAL / sealed TEST, two independent builds, `research/hod_entry/`), unfilled breaks −0.57 R; 9/25 caps 2 concurrent / 6 per day / $2K notional / −$300 day kill (Monday: 4 / 12 if the parity ledger `logs/hod_live_parity_ledger.csv` is clean); cell 1,438 (causal arming) pending; review `research/hod_entry/REVIEW.md` (fill rate and stop slippage overstated in the BT — measured live now); ladder `research/hod_entry/LADDER.md`; `--hod` | earlier research **CLOSED 9/18** (0/12 causal-filter cells); **exit lab 9/22: 35 more cells, every exit/filter/entry/slot variant within ±0.04R of the live rule (−0.22/−0.30R net) — the population carries no information** (`research/hod_exit_lab/REPORT.md`); dry run stays as a free forward instrument; status = `scripts/hod_dry_ledger.py` | dry_run MUST stay true; no more variants on this population; **order flow 9/24 (cells 1,393–1,395, `research/hod_ofi/REPORT.md`): the OFI lift was entirely cost — gross expectancy ≈ 0 in every bucket, filtering this population is closed**; next = a NEW signal definition with GROSS edge first, under its own PREREG |
...
`[HOD]` Telegram prefixes, `daily_bars` universe (2x wrappers IN since 9/5 for ORB, excluded for BF by name — **enforced in LIVE only since 9/25**: live BF had no guard 9/5–9/24 and bought RGTZ, a 2x short wrapper, −$600 on 9/23; fix 950dc4d = `trading/bf_universe_filter.is_bf_eligible` at `TradingEngine.on_stock_qualified`, grep `BF UNIVERSE:`).
