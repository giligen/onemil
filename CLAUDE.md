...
- **Semantics**: PM$ > $5.82M cut AND has_news → **2.0×**; PM$ high without news → **1.0×** (was 1.5 — that bucket is ~flat per-trade in all 3 eras); news without PM$ → 1.0 (news alone is negative — a headline nobody trades is a dud). Fail-open both channels: fetch failure → no boost, loud WARNING.
