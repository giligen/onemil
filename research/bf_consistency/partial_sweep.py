"""Partial-profit sweep on the faithful resim (relative tool): fraction x R level,
stop->BE after the partial, remainder to the plan target (2.5R) / EOD (legacy branch)."""
...
for r in (1.0, 1.5, 2.0, 2.5):
    for frac in (0.33, 0.5, 0.67):
        name=f'P_r{r}_f{frac}'
        cfg=copy.deepcopy(base); t=cfg['trading']; t['partial_profit']={'enabled':True,'r_multiple':r,'fraction':frac}
        cp=f'{SP}/cfg_ps_{name}.yaml'; yaml.safe_dump(cfg, open(cp,'w'), sort_keys=False)
        ...
        print(rows[-1])
df=pd.DataFrame(rows); df.to_csv(f'{OUT}/partial_sweep.csv', index=False); print(df.to_string(index=False)); print('SWEEP DONE')
