#!/usr/bin/env python3
"""S1-PASSIVE step 1 — the NBBO at the reopen instant for every candidate event.

Source and conventions reused from `research/fuckup_audit/P_cost/fetch_spreads.py`
(Alpaca SIP consolidated quotes; honesty rail = the last quote at or BEFORE the
decision instant). Decision instant here = `entry_t`, the open of the first
1-minute bar starting strictly after the LULD resume message — the first instant
the reopen print exists.

Emits, per event: the NBB/NBO in force at entry_t (used to price the passive short
limit) and the MEAN NBBO spread over the resume minute (the cost measurement the
PREREG declares; mean, not median).

Resumable: appends to entry_nbbo.csv, skips keys already present.
"""
...
OUT = f'{P}/entry_nbbo.csv'
FLOOR = 0.994          # the no-chase floor: reopen >= ref * FLOOR  (the rule)
...
def main() -> int:
    ...
    print(f'candidates {len(t):,}', flush=True)
    ...
    for i, r in enumerate(todo, 1):
        ...
        rec.update(symbol=r.symbol, day=r.day, halt_seq=r.halt_seq,
                   entry_t=r.entry_t.isoformat())
        try:
            ...
            # the resume minute = [entry_t, entry_t + 60s): the mean spread there is the cost measure
            minute = [(b, a) for ts, b, a in rows if r.entry_t <= ts < r.entry_t + pd.Timedelta(seconds=60)]
            if minute:
