...
d = d[(d.close >= 5) & (d.dvol20 >= 2e6) & d.prev_close.notna() & ~d.symbol.astype(str).str.match(r'^Z[VWX]ZZ|^ZZ')]
...
for side in ('loser', 'winner'):
    for sp in ('TRAIN', 'VAL', 'TEST'):
        ...
        lines.append(f"{side:6s} {sp:5s} n {len(x):5d} ({len(x) / x.day.nunique():.1f}/day) gross {x.ret.mean() * 1e4:+6.1f} bps median {x.ret.median() * 1e4:+6.1f} net {x.net.mean() * 1e4:+6.1f} t {x.net.mean() / (x.net.std() / np.sqrt(len(x))):+5.2f} hit {(x.net > 0).mean() * 100:4.1f}% | weeks green {int((w > 0).sum())}/{w.size} worst {w.min() * 100:+.1f}%")
    lines.append('')
