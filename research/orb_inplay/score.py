#!/usr/bin/env python3
"""orb_inplay step 4 — simulate + score the Zarattini in-play ORB book (PREREG)."""
import os,sys,json
...
def build():
    ...
    bars=pd.read_parquet(f'{D}/daybars.parquet')
...
def costs(t,hs_tab):
    """half-spread $/share per leg from the fitted (price band, clock) table."""
    def hs(px,m):
...
def stats(t,label,f):
    ...
    # >=3R winner gaps (weeks)
    wk=pd.to_datetime(t.day).dt.to_period('W')
    big=sorted(wk[t.net_r>=3.0].unique())
    allw=sorted(wk.unique())
    gap=0; last=-1
    idx={w:i for i,w in enumerate(allw)}
    bi=[idx[b] for b in big]
    prev=-1
    for b in bi: gap=max(gap,b-prev); prev=b
    gap=max(gap,len(allw)-1-prev)
    # green weeks vs count-matched null
    wpl=t.groupby(wk).pnl.sum(); gw=(wpl>0).mean()
    rng=np.random.default_rng(7); nulls=[]
    pv=t.pnl.values; wkc=wk.values
    for _ in range(2000):
