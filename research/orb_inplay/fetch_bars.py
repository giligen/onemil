#!/usr/bin/env python3
"""orb_inplay step 3 — 09:35-15:55 ET 1-min bars for the selected picks only."""
import os,sys,time
...
ET=ZoneInfo('America/New_York'); D=f'{ROOT}/research/orb_inplay'; OUT=f'{D}/daybars.parquet'
...
pd.concat(parts,ignore_index=True).to_parquet(OUT,index=False)
print(f'[bars] DONE {time.time()-t0:.0f}s',flush=True)
