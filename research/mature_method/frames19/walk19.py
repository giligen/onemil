#!/usr/bin/env python3
"""frames19 F56 — walk the PLACEBO populations P3 and P1 of the passive mirror short.

PREREG frames19/PREREG.md (frozen; FREEZE.md carries the git hash).  S is frames18/grid18.csv at
k=0.010 and is NOT re-walked here.  Entry/exit/cost stack identical to S (PREREG §2).  TEST sealed.
"""
import glob
import os
import sqlite3
...
def daily_panel(symbols):
    ...
    db['adv20'] = g['volume'].shift(1).rolling(20, min_periods=15).mean() \
        if False else g['volume'].transform(lambda s: s.shift(1).rolling(20, min_periods=15).mean())
...
def run():
    ...
    # candidate-signal hours per name-day (any family) -> removed from P3
    sig_hours = sw.groupby(['day', 'symbol']).hour.apply(lambda s: set(int(x) for x in s)).to_dict()
    ...
    # instrument class for the pool, once
    ...
    p3_rows, p1_rows = [], []
    stat = dict(s_rows=0, matched=0, no_match=0, p1_drawn=0, p1_kept=0)
    days = sorted(S.day.unique())
    for di, day in enumerate(days):
        g = S[S.day == day]
        # ---- P1 candidate universe for this session (causal, non-signal, shortable, non-wrapper)
        ...
        need = sorted(set(g.symbol) | set(want))
        qs = ','.join('?' * len(need))
        ...
        # ---------------------------------------------------------------- P3
        for sym in sorted(set(g.symbol)):
            x = byx.get(sym)
            ...
            skip = sig_hours.get((day, sym), set())
            for hour in HOURS:
                if hour in skip:
                    continue
                ...
                if hr is None or not hr['gate5'] or hr['price'] < 5:
                    continue
                rec = dict(pop='P3', day=day, symbol=sym, hour=hour, price=hr['price'],
                           entry_m=int(mv[hr['e']]))
        ...
        for sym, jobs in want.items():
            ...
            for hour, sig_ix in jobs:
                ...
                stat['p1_kept'] += 1
        if di % 40 == 0:
            print(f'  {di+1}/{len(days)} {day} -> P3 {len(p3_rows):,} P1 {len(p1_rows):,}',
                  flush=True)
    con.close()

    for nm, rows in (('p3', p3_rows), ('p1', p1_rows)):
        z = pd.DataFrame(rows)
        z['split'] = np.where(z.day < '2026-01-01', 'TRAIN', 'VAL')
        assert z.day.max() < TEST_FROM
        z.to_csv(f'{D}/{nm}19.csv', index=False)
        print(f'[{nm}] {len(z):,} rows, {z.day.nunique()} sessions, touch rate (w=10) '
    ...
    print(f'[match] S rows {stat["s_rows"]:,}; matched {stat["matched"]:,} '
          ...
          f'P1 drawn {stat["p1_drawn"]:,}; P1 kept after gate5/price rails {stat["p1_kept"]:,}',
          flush=True)
    return 0
