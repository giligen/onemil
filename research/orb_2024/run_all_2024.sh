...
LOG=research/orb_2024/run_all.log
: > "$LOG"
echo "=== START $(date -u +%H:%M:%S) ===" >> "$LOG"
for CELL in 1415 1416; do
  echo "--- build $CELL start $(date -u +%H:%M:%S)" >> "$LOG"
  Y24_CELL=$CELL python3 research/orb_2024/build_features_2024.py >> "$LOG" 2>&1
  echo "--- build $CELL rc=$? done $(date -u +%H:%M:%S)" >> "$LOG"
done
for CELL in 1415 1416; do
  FCSV=$(ls research/orb_2024/out_${CELL}/*.csv 2>/dev/null | grep -v VERSION | head -1)
  echo "--- pipeline $CELL features=$FCSV start $(date -u +%H:%M:%S)" >> "$LOG"
  if [ -n "$FCSV" ]; then
    ORB_BT_FEATURES_CSV=$FCSV ORB_BT_BOOK_OUT=research/orb_2024/book_${CELL}.csv ORB_CATALYST_VETO=0 \
...
python3 research/orb_2024/score.py >> "$LOG" 2>&1
