...
STOP_SLIP = 0.003    # report-only: stop exits filled 30 bps worse


def stop_slip_net(f):
    """Net R of fills with every stop exit (stop / stop_bar) filled STOP_SLIP worse; exit leg cost unchanged."""
    st = f.why.isin(['stop', 'stop_bar'])
    return f.net_R - np.where(st, f.exit_price * STOP_SLIP / f.R, 0.0)


def _legs(v):
    """The frozen VAL pass-bar legs of a score dict: list of (name, passed, value-text)."""
    return [('mean ≥ +0.10', v['mean_R'] >= 0.10, f'{v["mean_R"]:+.3f}'), ('t ≥ 2', v['t'] >= 2, f'{v["t"]:.2f}'),
...
def report(res, n_nobars):
    """REPORT_1438.md, SHORT format (owner): one row per split + <= 5 lines (overlap with 1,427, 1,438-only
    outcome, 30 bps stop-slip variant, tick-rv variant, scope). VAL bar applied; TEST not read."""
    sc = {}
    for v in ('causal', 'tick_rv'):
        for sp in ('TRAIN', 'VAL'):
            allv = res[(res.variant == v) & (res.split == sp)]
            sc[(v, sp)] = sr.score(allv[allv.status != 'not_armed'].copy(), allv)
    lines = ['# REPORT — cell 1,438: causal arming (PREREG_1438.md + amendment) — TEST NOT read', '',
    ...
    for sp, cn in (('TRAIN', 'TRAIN-H2'), ('VAL', 'VAL')):
        ...
        lines.append(f'| {cn} | {v["fills"]} ({v["fill_rate"]*100:.1f} %) | {v["mean_R"]:+.3f} | {v["t"]:.2f} | '
                     f'{v["extop5"]:+.3f} | {v["fills_wk"]:.1f} | {v["coverage"]*100:.1f} % / {v["miss_gap"]*100:.1f} pp '
                     f'| {verdict} |')
    c27 = pd.read_csv(C1427_CSV, dtype={'day': str, 'symbol': str}, keep_default_na=False, na_values=[''])
    c27 = c27[c27.status == 'fill'][['day', 'symbol', 'split', 'net_R']].rename(columns={'net_R': 'R27'})
    ov, sl, tk = [], [], []
    for sp, cn in (('TRAIN', 'TRAIN-H2'), ('VAL', 'VAL')):
        f = res[(res.variant == 'causal') & (res.split == sp) & (res.status == 'fill')]
        m = f[['day', 'symbol', 'net_R']].merge(c27[c27.split == sp], on=['day', 'symbol'], how='outer', indicator=True)
        b, lo, ro = m[m._merge == 'both'], m[m._merge == 'left_only'], m[m._merge == 'right_only']
        ov.append(f'{cn}: both {len(b)} ({b.net_R.mean():+.3f} here / {b.R27.mean():+.3f} in 1,427), 1,438-only '
                  f'{len(lo)} ({lo.net_R.mean():+.3f}), 1,427-only {len(ro)} ({ro.R27.mean():+.3f})')
        sl.append(f'{cn} {stop_slip_net(f).mean():+.3f}')
        t = sc[('tick_rv', sp)]
        tk.append(f'{cn} {t["fills"]} fills {t["mean_R"]:+.3f} (t {t["t"]:.2f})')
    lines += ['', f'* Overlap with 1,427 fills (day, symbol) — {"; ".join(ov)}.',
              f'* 30 bps stop-slip (every stop exit 0.3 % worse): mean net R {", ".join(sl)}.',
              f'* Report-only rv incl. bar j+1 tick volume to the fill: {"; ".join(tk)}.',
              f'* Scope: superset symbol-days with >= 1 armed bar whose next bar crosses; minute bars cache.db + '
              f'bars_sip.db (amendment); {n_nobars} superset symbol-days had < K+2 bars in both. Coverage / gap use a '
              f'proxy winner (fill at the trigger on the first crossing armed bar).', '']
    with open(REPORT_MD, 'w') as fh:
        fh.write('\n'.join(lines))
    log('[report] VAL ' + '; '.join(f'{n}={x}' for n, _, x in _legs(sc[('causal', 'VAL')])))
