...
| 7 | **1,441 DONE 00:30 UTC 9/26 — FAIL (`RESULT_1441.md`): VAL −0.17 R t −3.4, ex-top-5 % −0.28, TRAIN-H2 +0.02 t −0.4; population = day after a mover day (74 % no prior-session bars, disclosed). 1,435 / 1,432 not run (same order, same population → not funded after 7 FAILs)** — 1,435 / 1,432 / 1,441 (NEW) | other levels with the same order: pre-market high; opening-range high (ORB tick data exists); prior-day high | existing bars/ticks (+ PMH pre-market bars) | mean net R ≥ +0.10 both holdouts each |
