...
minute. Resumable; output research/ignition_news/news_25H2.csv"""
...
t=pd.read_csv(f'{ROOT}/research/ignition_capcheck/trades_25H2.csv')
days=sorted(t.day.unique()); state=f'{OUT}/state_25H2.json'
done=json.load(open(state)) if os.path.exists(state) else {}
arts=[]
if os.path.exists(f'{OUT}/articles_25H2.csv'):
    arts=pd.read_csv(f'{OUT}/articles_25H2.csv').to_dict('records')
...
for i,day in enumerate(days):
    ...
    pd.DataFrame(arts).to_csv(f'{OUT}/articles_25H2.csv', index=False)
...
if len(A):
    ...
    out.to_csv(f'{OUT}/news_25H2.csv', index=False)
